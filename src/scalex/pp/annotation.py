...
def remove_genes_without_annotation(adata, by="gene_name", exclude_mt=True):
    ...
    if exclude_mt:
        indices = [
            i for i, name in enumerate(adata.var.gene_name) if not str(name).startswith(tuple(["ERCC", "MT-", "mt-"]))
        ]
...
def format_rna(rna, gtf=os.path.expanduser("~/.scalex/gencode.v38.annotation.gtf.gz"), up=1000, down=100, force=False):
...
def read_gencode_transcripts(
    gtf_dir=GTF_FILE,
    release="latest",
    transcript_choice="MANE_Select",
    annotation_set="basic",
    gene_type=r"lncRNA|protein_coding|IG_.*_gene|TR_.*_gene",
    attributes=None,
