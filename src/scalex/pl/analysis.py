...
def annotate(
    ...
    sc.pl.rank_genes_groups_dotplot(
        adata, n_genes=5, cmap="coolwarm", key=cell_type, standard_scale="var", figsize=(22, 5), dendrogram=False
    )
        ...
        if go:
            for option in options:
                ...
                if out_dir is not None:
                    ...
                    go_results = (
                        go_results.sort_values("Adjusted P-value", ascending=False).groupby("cell_type").head(10)
                    )
                    go_results[["Gene_set", "Term", "Overlap", "Adjusted P-value", "Genes", "cell_type"]].to_csv(
                        out_dir + f"/{option}_go_results_{n_top}.csv"
                    )
                plt.show()
