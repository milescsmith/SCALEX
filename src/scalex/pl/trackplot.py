...
def set_trackplot_label(plot, labels):
    ...
    if len(plot.axes) != len(labels):
        raise ValueError(
            f"set_trackplot_label(): Plot has {len(plot.axes)} facets, but {len(labels)} labels were provided"
        )
...
def trackplot_create_arrow_segs(data, region, size=50, head_only=False):
    ...
    for _, row in data.iterrows():
        ...
        if len(endpoints) > 1:
            new_arrow = pd.DataFrame({"start": endpoints[:-1], "end": endpoints[1:]})
...
def trackplot_coverage(
    ...
    for ax, group in zip(axes, groups):
        ...
        ax.fill_between(group_data["pos"], group_data["normalized_insertions"], color=colors[group], linewidth=0)
        ...
        # Group label on left
        ax.text(-0.0, 0.5, group, transform=ax.transAxes, va="center", ha="right", fontsize=11)

        # Add per-plot range label on top-left
        ax.text(
            0.01,
            0.97,
            f"[{int(global_ymin)}-{int(global_ymax)}]",
...
def trackplot_gene(
    transcripts, region, exon_size=2.5, gene_size=0.5, label_size=11 * 0.8, track_label="Genes", return_data=False
):
    ...
    if len(arrows_df) > 0 and len(consolidated_df) > 0:
        ...
        if "gene_name" in arrows_df.columns and "gene_name" in consolidated_df.columns:
            arrows_df = arrows_df.merge(consolidated_df[["gene_name", "y"]], on="gene_name", how="left")
    ...
    for _, row in consolidated_df.iterrows():
        ...
        ax.hlines(y=row["y"], xmin=row["Start"], xmax=row["End"], linewidth=row["size"], color=color)

        # Plot individual exons on top
        exons = row["exons"]
        for _, exon in exons.iterrows():
            ax.hlines(y=row["y"], xmin=exon["Start"], xmax=exon["End"], linewidth=exon_size, color=color)
    ...
    for _, row in consolidated_df.iterrows():
        ...
        ax.text(label_x, label_y, row["gene_name"], ha="center", va="bottom", fontsize=label_size, color="black")
...
def trackplot_genome_annotation(
    loci,
    region,
    color_by=None,
    colors=None,
    ...
    df = df[(df["Chromosome"] == chrom) & (df["End"] > start) & (df["Start"] < end)].copy()
    ...
    for _, row in df.iterrows():
        ...
        ax.hlines(y=row["y"], xmin=row["Start"], xmax=row["End"], linewidth=annotation_size, color=color)
    ...
    if threshold is not None and score_column and score_column in df.columns and len(df) > 0:
        ...
        if max_score > min_score:
            ...
            if 0 <= threshold_normalized <= 1:
                ax.axhline(y=threshold_normalized, color="red", linestyle="--", linewidth=2, alpha=0.8, zorder=10)
                # Add threshold label
                ax.text(
                    start + (end - start) * 0.02,
                    threshold_normalized + 0.02,
    ...
    if score_column and score_column in df.columns and len(df) > 0:
        ...
        if max_score > min_score:
            tick_labels = [f"{min_score + pos * (max_score - min_score):.1f}" for pos in tick_positions]
        else:
...
def trackplot_scalebar(region, font_pt=11):
    ...
    ax.text(bar_start - width * 0.01, 0, f"{bar_width_kb:.0f}k", fontsize=font_pt, ha="right", va="center")
    ...
    plot = fig
    return wrap_trackplot(
        plot, height=font_pt * 1.1 / 72, region={"chr": chrom, "start": start, "end": end}, keep_vertical_margin=False
    )
    # return plot
...
def trackplot_combine(
    ...
    if return_data:
        ...
        coverage_data = trackplot_coverage(
            frag_file, region, cell_groups, cell_counts, bins, clip_quantile, colors, return_data=True
        )
    ...
    # Add region text
    scalebar_ax.text(start, 0, f"{chrom}: {start:,} - {end:,}", fontsize=11, ha="left", va="center")
    ...
    # Distance text on the left of the scale bar (in kb)
    bar_width_kb = bar_width / 1000
    scalebar_ax.text(bar_start - width * 0.01, 0, f"{bar_width_kb:.0f}k", fontsize=11, ha="right", va="center")
    ...
    for i, group in enumerate(groups):
        ...
        ax.fill_between(bin_centers, norm_cov, color=colors[group], linewidth=0)
    ...
    if has_loci:
        ...
        loci_df = loci_df[(loci_df["Chromosome"] == chrom) & (loci_df["End"] > start) & (loci_df["Start"] < end)].copy()
        ...
        if not loci_df.empty:
            ...
            for _, row in loci_df.iterrows():
                ...
                if loci_color_by and loci_color_by in loci_df.columns:
                    if loci_df[annotation_color_by].dtype in ["int64", "float64"]:
                        ...
                        color = plt.cm.viridis(
                            (row[annotation_color_by] - loci_df[annotation_color_by].min())
                ...
                annotation_ax.hlines(y=row["y"], xmin=row["Start"], xmax=row["End"], linewidth=2.5, color=color)

            # Add threshold line if specified
            if loci_threshold is not None and loci_score_column and loci_score_column in loci_df.columns:
                # Convert threshold to normalized position
                if max_score > min_score:
                    threshold_normalized = (loci_threshold - min_score) / (max_score - min_score)
                    # Only draw if threshold is within the score range
                    if 0 <= threshold_normalized <= 1:
                        annotation_ax.axhline(
                            y=threshold_normalized, color="red", linestyle="--", linewidth=2, alpha=0.8, zorder=10
                        )
    ...
    if has_loops:
        ...
        loop_df = loop_df[
            (loop_df["Chromosome"] == chrom) & ((loop_df["End"] > start) & (loop_df["Start"] < end))
        ].copy()
    ...
    if has_transcripts:
        ...
        if consolidated_data:
            ...
            for _, row in consolidated_df.iterrows():
                ...
                # Plot the main gene line
                gene_ax.hlines(y=row["y"], xmin=row["Start"], xmax=row["End"], linewidth=gene_size, color=color)

                # Plot individual exons
                exons = row["exons"]
                for _, exon in exons.iterrows():
                    gene_ax.hlines(y=row["y"], xmin=exon["Start"], xmax=exon["End"], linewidth=exon_size, color=color)

                # Add gene label
                label_x = (row["Start"] + row["End"]) / 2
                label_y = row["y"] + 0.1
                gene_ax.text(
                    label_x, label_y, row["gene_name"], ha="center", va="bottom", fontsize=label_size, color="black"
                )
