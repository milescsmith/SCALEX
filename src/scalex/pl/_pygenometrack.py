...
def plot_tracks(
    ...
    # atac bigwig
    cell_type_kws = {"file_type": "bigwig"}
    ...
    if meta_gr is not None:
        ...
        for i, c in enumerate(cell_types):
            ...
            if not os.path.isfile(os.path.join(fig_dir, "bw", f"{c}.bw")):
                meta_gr.to_bigwig(
                    os.path.join(fig_dir, "bw", f"{c}.bw"), chromosome_sizes=chromsizes, value_col=d, rpm=False
                )
...
class ShapPlotter:
    def __init__(self, dir, sample, fold=0, min_value=-0.025, max_value=0.1, shap_h5=None, shap_bed=None):
        self.dir = dir
        self.sample = sample
        self.fold = fold
