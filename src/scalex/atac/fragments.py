...
def make_peak_matrix(fragment_file, peaks_df, barcode_list, output_h5ad=None, n_jobs=32, meta=None):
    """
...
from scalex.atac.bedtools import intersect_bed, extend_bed, subtract_bed, edge_index_to_coo
...
def reindex_peak(atac, new_coord, dist=0, power_law=False, blacklist=None):
    ...
    if power_law:
        ...
        adj = edge_index_to_coo(edges, data=link_index["score"], shape=(atac.shape[0], len(new_coord.shape[0])))
    else:
...
## Powerlaw from ABC
def get_powerlaw_at_distance(distances, gamma=0.87, min_distance=5000, scale=None):
    assert gamma > 0
...
def get_gtf(genome="hg38", drop_by="gene_name"):
    ...
    if not os.path.exists(gtf_file):
        ...
        os.system(
            f"wget -O {gtf_file} https://ftp.ebi.ac.uk/pub/databases/gencode/Gencode_human/release_{version}/gencode.v{version}.annotation.gtf.gz"
        )
...
if __name__ == "__main__":
    ...
    if args.command == "gene":
        ...
        if args.method == "pyranges":
            peak_matrix = make_peak_matrix(
                args.fragment, peaks, barcode_list=barcode, meta=meta, output_h5ad=args.output, n_jobs=args.n_jobs
            )
