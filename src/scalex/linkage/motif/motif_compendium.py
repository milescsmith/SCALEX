...
def main(args):
    ...
    out_dir = os.path.join(
        args.out_dir, "all_data"
    )  #'/users/leixiong/projects/imac_igvf/results/motif_compendium/all_data/'
    ...
    selin_h5_file = (
        args.motif_dir + "/selin/selin.motif_compendium.avg.h5"
    ...
    utils_analysis.label_from_pfms(
        mc_avg, vierstra_meme_file, "vierstra_similarity", "vierstra_match", max_chunk=1000
    )  # , use_gpu=True)
    vierstra_metadata = pd.read_table(vierstra_meta_file)
    mc_avg.metadata = mc_avg.metadata.merge(
        vierstra_metadata[["motif_id", "tf_name"]], left_on="vierstra_match", right_on="motif_id", how="left"
    )
    ...
    mc_avg.assign_clusters_from_other(
        selin_mc, "annotation", "selin_similarity", "selin_match", max_chunk=1000
    )  # , use_gpu=True)
    ...
    for index, row in mc_avg.metadata.iterrows():
        if row["selin_similarity"] > threshold:
            ...
            cluster_labels_dict[index] = "Unknown"
    ...
    mc_metadata["name"] = mc_metadata["name"].str.replace(
        r".*-(pos|neg)\.pattern_", r"\1_patterns.pattern_", regex=True
    )
    ...
    utils_analysis.cluster_grouping_upset_plot(
        mc, leiden_name, "model", os.path.join(cluster_path, "upset_plot_celltype_vs_cluster.png"), min_subset_size=3
    )

    ### Save and export
    mc_avg.summary_table_html(
        os.path.join(cluster_path, f"all_data.summary_table.{leiden_name}.html"), mc_avg.metadata.columns, 16
    )
    ...
    utils_analysis.export_modisco(
        mc_avg, "name", os.path.join(cluster_path, f"all_data.motif_compendium.avg.{leiden_name}.h5")
    )
    mc_avg.metadata.to_csv(
        os.path.join(cluster_path, f"all_data.motif_compendium.avg.metadata.{leiden_name}.tsv"), sep="\t", index=False
    )

    mc_avg_annotated_only_metadata = mc_avg.metadata.loc[~mc_avg.metadata["annotation"].str.startswith("Unknown")][
        ["name", "annotation"]
    ]
    mc_avg_annotated_only_metadata.to_csv(
        os.path.join(cluster_path, f"all_data.motif_compendium.avg.metadata.annotated_only.{leiden_name}.tsv"),
        sep="\t",
        header=False,
        index=False,
    )
    ...
    vierstra_metadata = pd.read_table(vierstra_meta_file)
    vierstra_subset = vierstra_metadata.loc[
        vierstra_metadata.apply(
            lambda x: x["tf_name"] + "_" + x["motif_id"] in metadata["vierstra_match"].values, axis=1
        )
    ]
    ...
    hocomoco_meme_subset = {
        key: hocomoco_meme[key] for key in hocomoco_meme.keys() if key in metadata["hocomoco_match"].values
    }
    ...
    metadata["modisco_fwd"] = path_to_image_link(
        logo_link_base + f"/modisco/all_data/{leiden_name}/" + metadata["name"] + ".cwm.fwd.png"
    )
    metadata["modisco_rev"] = path_to_image_link(
        logo_link_base + f"/modisco/all_data/{leiden_name}/" + metadata["name"] + ".cwm.rev.png"
    )
    ...
    metadata.rename(columns={"name": "modisco_pattern", "annotation": "auto_annotation"}, inplace=True)
    ...
    metadata.to_csv(
        os.path.join(cluster_path, f"all_data.motif_compendium.avg.metadata.{leiden_name}.logos.tsv"),
        sep="\t",
        index=False,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--motif_dir",
        type=str,
        default="/oak/stanford/groups/akundaje/soumyak/motifs/latest/",
        help="Directory containing motif files",
    )
