...
from scalex.analysis import flatten_dict
# from scalex.linkage.utils import row_wise_correlation


class Linkage:
    def __init__(
        self, rna=None, atac=None, rna_avg=None, atac_avg=None, groupby="cell_type", gtf=None, up=100_000, down=100_000
    ):
        self.rna = rna
        self.atac = atac
    ...
    def find_overlapping_peak_gene_linkage(self, genes_deg, peaks_deg, threshold=0.7):
        ...
        gene_peak_pair = [
            (genes[i], peaks[i]) for i, _ in enumerate(genes) if genes[i] in genes_deg and peaks[i] in peaks_deg
        ]
