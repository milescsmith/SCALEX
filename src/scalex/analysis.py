...
cell_type_markers_human = {
    ...
    "Foamy": [
        ...
        "LGALS3",
        "LGALS1",
    ],  # IL4I1# APOE covers SPP1, LPL is a subset of SPP1, while 'LGMN' is exclusive to SPP1+ macrophages
    ...
    "Proliferating": [
        ...
        "TUBB",
        "SMC2",
    ],  # , 'CDK1', 'CCNB1', 'CCNB2', 'CCNA2', 'CCNE1', 'CDK2', 'CDK4', 'CDK6'],
    "Inflammatory": [
        "NFKBIA",
        "IL1B",
        "CXCL2",
    ...
    "NK activating": [
        ...
        "NR1H3",
        "TIMD4",
    ],  # Membrane Lipid Catabolic Process, Interleukin-12 Production, Apoptotic cell clearance
...
def enrich_module(adata, gene_sets):
    ...
    for k, v in gene_sets.items():
        # print(k)
        ov.single.geneset_aucell(adata, geneset_name=k, geneset=v)  # pathway_dict[geneset_name])
...
def enrich_analysis(
    gene_names, organism="hsapiens", gene_sets="GO_Biological_Process_2023", cutoff=0.05, **kwargs
...
def enrich_and_plot(
    gene_names, organism="hsapiens", gene_sets="GO_Biological_Process_2023", cutoff=0.05, add="", out_dir=None, **kwargs
):
    ...
    if out_dir is not None:
        ...
        plt.savefig(os.path.join(out_dir, "go_results.pdf"))
    plt.show()
...
def get_markers(
    ...
    for cluster in clusters:
        ...
        # Apply filters
        filtered = df[(df["pvals_adj"] < pval_cutoff) & (df["logfoldchanges"] > logfc_cutoff)].copy()
...
def find_gene_program(
    adata, groupby="cell_type", processed=False, n_clusters=None, top_n=300, filter_pseudo=True, **kwargs
):
    """
    ...
    markers = get_markers(
        adata, groupby=groupby, processed=processed, top_n=top_n, filter_pseudo=filter_pseudo, **kwargs
    )
    for cluster, genes in markers.items():
...
def find_peak_program(
    ...
    **kwargs,
    ...
    """
    return find_gene_program(
        adata,
        groupby=groupby,
        processed=processed,
        top_n=top_n,
        filter_pseudo=filter_pseudo,
...
def find_consensus_program(
    ...
    **kwargs,
    ...
    if across is not None:
        ...
        print(
            "There are {} {set_type}s with at least {} occurrences".format(
                len(markers_list), occurance, set_type=set_type
            )
        )
...
def merge_all_go_results(
    ...
    add_ref=False,
    union=True,
    reference="GO_Biological_Process_2023",
    organism="human",
):
    """
